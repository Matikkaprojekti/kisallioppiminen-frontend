...
export default function NewInstanceForm() {
  const form = ({
    ...
    const submitForm = (event: FormEvent) => {
      ...
      return createTeacherCourse(instance).then(() =>
        setFormValue({
          name: '',
          coursematerial_name,
          version,
          ...
          <table>
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <select
                    value={coursematerial_name}
                    onChange={e =>
                      setFormValue({
                        coursematerial_name: e.target.value,
                        version: pageState.courses.filter(c => c.courseName === e.target.value)[0].courseContent[0].version
                      })
                    }
                  >
